...
- **Markdown**: Split based on headers (e.g., #, ##, ###) or using the `ExperimentalMarkdownSyntaxTextSplitter` which preserves whitespace and extracts metadata like code blocks.
