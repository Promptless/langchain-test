...
- **Markdown**: Split based on headers (e.g., #, ##, ###). The `ExperimentalMarkdownSyntaxTextSplitter` preserves whitespace and formatting, making it suitable for documents where maintaining the original structure is crucial.
