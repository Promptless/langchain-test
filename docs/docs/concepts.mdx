...
LangChain now includes an `ExperimentalMarkdownSyntaxTextSplitter` that retains the original whitespace and formatting of Markdown text while extracting structured metadata such as headers and code blocks. This splitter is particularly useful for maintaining the structure of Markdown documents, allowing for more flexible manipulation of the resulting document sections. It also supports splitting on horizontal rules and can be customized with specific headers to split on.
